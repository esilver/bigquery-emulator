...
func (h *uploadContentHandler) Handle(ctx context.Context, r *uploadContentRequest) error {
	...
	// A load against a table with no schema (e.g. a schemaless Parquet load,
	// since Parquet schema inference is not implemented) would dereference a
	// nil schema below and on the per-format paths. Reject it with a clear
	// 4xx instead of panicking (the panic is otherwise recovered as a 500).
	if tableContent.Schema == nil {
		return errInvalid(fmt.Sprintf("table %q has no schema; provide configuration.load.schema", tableRef.TableId))
	}
	...
	case "PARQUET":
		...
		// Reconstructing each row into an interface{} yields raw integers for
		// TIMESTAMP/DATE/TIME leaves; convertParquetTemporal turns them into the
		// Go values AddTableData binds correctly for each temporal column.
		units := parquetLeafUnits(reader.Schema())
...
func (h *jobsGetQueryResultsHandler) Handle(ctx context.Context, r *jobsGetQueryResultsRequest) (*sqlvalues.GetQueryResultsResponse, error) {
	...
	if response == nil {
		return &sqlvalues.GetQueryResultsResponse{
	...
	rows := sqlvalues.Format(response.Schema, response.Rows, r.useInt64Timestamp)
...
func (h *jobsInsertHandler) tableDefFromQueryResponse(tableID string, response *sqlvalues.QueryResponse) (*types.Table, error) {
	columns := []*types.Column{}
...
func (h *jobsInsertHandler) exportToGCSWithObject(ctx context.Context, response *sqlvalues.QueryResponse, extract *bigqueryv2.JobConfigurationExtract, writer *storage.Writer) (e error) {
	defer func() {
...
func (h *jobsInsertHandler) handleSchemaDDL(ctx context.Context, r *jobsInsertRequest, ddl schemaDDL) (*bigqueryv2.Job, error) {
	...
	job := r.job
	// A CREATE/DROP SCHEMA query job can arrive with no jobReference, so
	// synthesize one before reading JobId to keep the handler from dereferencing
	// a nil pointer.
...
func (h *jobsInsertHandler) addQueryResultToDynamicDestinationTable(ctx context.Context, tx *connection.Tx, r *jobsInsertRequest, response *sqlvalues.QueryResponse) (*bigqueryv2.TableReference, error) {
	projectID := r.project.ID
...
func (h *jobsQueryHandler) Handle(ctx context.Context, r *jobsQueryRequest) (*sqlvalues.QueryResponse, error) {
	...
	formattedRows := sqlvalues.Format(response.Schema, response.Rows, r.useInt64Timestamp)
	pageToken := ""
	if r.hasMaxResults {
		end := r.maxResults
...
func (h *tabledataListHandler) Handle(ctx context.Context, r *tabledataListRequest) (*sqlvalues.TableDataList, error) {
	conn, err := r.server.connMgr.Connection(ctx, r.project.ID, r.dataset.ID)
	if err != nil {
		return nil, fmt.Errorf("failed to get connection: %w", err)
	}
	...
	return &sqlvalues.TableDataList{
		Rows:      sqlvalues.Format(response.Schema, response.Rows, r.useInt64Timestamp),
		TotalRows: response.TotalRows,
	}, nil
}
