...
func TestResumableUploadSessionURLHonorsForwardedHeaders(t *testing.T) {
	ts := newDBTTestServer(t)
	mustCreateDataset(t, ts.URL, "d16")

	code, location, body := startResumableSession(t, ts.URL, "internal-hop:9050", "resumable-fwd-1", map[string]string{
		"X-Forwarded-Host":  "bq.example.com:8443, internal-hop:9050",
	...
	// The first X-Forwarded-Host entry is the client-facing authority.
	if parsed.Scheme != "https" || parsed.Host != "bq.example.com:8443" {
		t.Errorf("session URL = %s, want scheme https and host bq.example.com:8443", location)
	}
}
