...
| Storage backend | ✅ | Embedded `googlesqlite` backend. SQLite and DuckDB-backed builds use different on-disk formats. |
...
| In-memory storage | ✅ | |
| Persisted file storage | ✅ | Backend-specific database file; SQLite and DuckDB builds are not interchangeable. |
